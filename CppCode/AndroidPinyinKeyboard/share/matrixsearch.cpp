...
size_t MatrixSearch::search(const char *py, size_t py_len, Position *pos) {
	this->userPos = pos;
	if (!inited_ || NULL == py)
		return 0;
...
bool MatrixSearch::add_char(char ch) {
  if (!prepare_add_char(ch))
    return false;
  return add_char_qwerty_new();
}


bool MatrixSearch::add_char_qwerty_new() {
	if (kPrintDebug1) {
		printf("add char qwerty begin:\n");
	}
	...
	for (uint16 ext_len = kMaxPinyinSize + 1; ext_len > 0; ext_len--) {
		...
		if (kPrintDebug1) {
			...
			memcpy(target, pys_ + oldrow, ext_len);
			target[ext_len] = '\0';
			printf("try to add probability:%s\t\n", target);
		}
		...
		/*if (0 == matrix_[oldrow].mtrx_nd_num && !dmi_c_phrase_)
			continue;*/// if not commented then uaikeyi is not valid
		...
		spl_idx = spl_parser_->get_splid_by_str(pys_ + oldrow,
			ext_len, &is_pre);
		// here we need to use the information of user position to 
		// calculate a list of the most probable spelling ids;
		// pos : start from userpos oldrow-th, length: ext_len
		// also, we need to return the score (probability) of each 
		// item, which will be used in combination with language model
		///////////////////////////////////////////////////////////
		//for simplicity, currently we treat half spelling str as accurate
		//input without correction
		bool isHalf = false;
		if (spl_idx != 0) {
		...
		uint16 splids[400];
		double splscores[400];

		int spl_can_nums = 1;
		splids[0] = spl_idx;
		splscores[0] = 0.0;
		if (!isHalf) {
			//match some spelling ids with equal length
			SpellingSearch* spl_seasrch_ = new SpellingSearch(spl_parser_, spl_trie_);
			spl_can_nums = spl_seasrch_->searchFullSpellingIds(touch_model_, splids, splscores, userPos + oldrow, pys_+oldrow, ext_len);
		}
		///////////////////////////////////////////////////////////
		if (is_pre)
			spl_matched = true;
		...
		/*if (0 == spl_idx)
			continue;*/
		if (spl_can_nums == 1 && splids[0] == 0 /*spl_idx == 0*/) {
			continue;
		}
		if (0 == spl_can_nums) {
			continue;
		...
		for (int spl_pos = 0; spl_pos < spl_can_nums; spl_pos++) {
			spl_idx = splids[spl_pos];
			if (kPrintDebug1) {
				const char* ch = SpellingTrie::get_instance().get_spelling_str(spl_idx);
				printf("append %s\n", ch);
			}
			...
			for (PoolPosType dmi_pos = matrix_[oldrow].dmi_pos;
				...
				DictMatchInfo *dmi = dmi_pool_ + dmi_pos;
				if (kPrintDebug1) {
					const char* swn = SpellingTrie::get_instance().get_spelling_str(dmi->spl_id);
					printf("?? try to for loop: %s\n", swn);
				}
				...
				if (kPrintDebug1) {
					printf("=========dep start ========\n");
					for (int kk = 0; kk < dep_->splids_extended + 1; kk++) {
						printf("%d: %s, ext_len: %d\n", dep_->splids[kk], SpellingTrie::get_instance().get_spelling_str(dep_->splids[kk]),
							dep_->ext_len);
					}
					printf("=========dep end===========\n");
				}
				...
				/*for (int aa = 0; aa < dmi_pool_used_; aa++) {
					const char* cccc = SpellingTrie::get_instance().get_spelling_str(dmi_pool_[aa].spl_id);
					printf("dmi: %d, %s, from: %d\n", aa, cccc,  dmi_pool_[aa].dmi_fr);
				}*/
				...
				if (lpi_total_ > 0) {
					...
					for (PoolPosType mtrx_nd_pos = matrix_[fr_row].mtrx_nd_pos;
						...
						int isFullId = 1;
						if (dmi != NULL) {
							isFullId = dmi->all_full_id;
							if (spl_trie_->is_half_id(spl_idx)) {
								isFullId = 0;
							} 
						}
						extend_mtrx_nd(mtrx_nd, lpi_items_, lpi_total_,
							dmi_pool_used_ - new_dmi_num, pys_decoded_len_, splscores[spl_pos]+ (1-isFullId)*500);
	...
	if (dmi_c_phrase_)
		return true;
	if (kPrintDebug1) {
		printf("lpiiiiiiiiiiiiiiiiiii::%d\n", lpi_total_);
		printf("add char qwerty end\n");
	}
	return true;
...
void MatrixSearch::prepare_candidates_new() {
	...
	lpi_total_ = 0;
	for (int i =0; i<matrix_[pys_decoded_len_].mtrx_nd_num; i++) {
		lma_size = lma_size_max;
		MatrixNode *node = mtrx_nd_pool_ + matrix_[pys_decoded_len_].mtrx_nd_pos + i;
		...
		do {
			id_num--;
			if (idxs[id_num] == 0) continue;
			char16 str[kMaxLemmaSize + 1];
			uint16 str_len = get_lemma_str(idxs[id_num], str, kMaxLemmaSize + 1);
			if (str_len > 0 && (kMaxLemmaSize + 1 - ret_pos + fixed_hzs_ > str_len)) {
				if (ret_pos >= fixed_hzs_) {
					utf16_strncpy(fullsent + ret_pos - fixed_hzs_, str, str_len);
				}
				ret_pos += str_len;
			}
			else {
		...
		sent_len = ret_pos - fixed_hzs_;
		fullsent[ret_pos - fixed_hzs_] = (char16)'\0';
		pfullsent = fullsent;
...
size_t MatrixSearch::extend_mtrx_nd(MatrixNode *mtrx_nd, LmaPsbItem lpi_items[],
	...
	size_t res_row, double pinyin_score) {
	assert(NULL != mtrx_nd);
	...
	for (size_t pos = 0; pos < lpi_num; pos++) {
		float score = mtrx_nd->score + lpi_items[pos].psb + static_cast<float>(pinyin_score);
...
char16* MatrixSearch::get_candidate0(char16 *cand_str, size_t max_len,
  ...
  MatrixNode *mtrx_nd = mtrx_nd_pool_ + matrix_[pys_decoded_len_].mtrx_nd_pos;
  if (kPrintDebug3) {
	  printf("??????%d\n", matrix_[pys_decoded_len_].mtrx_nd_num);
	  for (int i = 0; i < matrix_[pys_decoded_len_].mtrx_nd_num; i++) {
		  printf("================\n");
		  mtrx_nd = mtrx_nd_pool_ + matrix_[pys_decoded_len_].mtrx_nd_pos + i;
		  while (mtrx_nd != NULL) {
  ...
  mtrx_nd = mtrx_nd_pool_ + matrix_[pys_decoded_len_].mtrx_nd_pos;
  if (kPrintDebug0) {
